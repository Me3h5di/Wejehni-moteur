const GROUPES_MOTSCLES = [
  {
    domaine: "Sciences Médicales",
    label_ar: "العلوم الطبية",
    mots: [
      "anatomie",
      "biologie humaine",
      "biologie médicale",
      "chimie du médicament",
      "chirurgie buccale",
      "clinique",
      "diagnostic",
      "esthétique dentaire",
      "hôpital",
      "industrie pharmaceutique",
      "médecine générale",
      "orthodontie",
      "santé animale",
      "sauver des vies",
      "soigner",
      "sécurité alimentaire",
      "urgence médicale",
      "élevage",
    ],
  },
  {
    domaine: "Paramédical",
    label_ar: "شبه الطبي",
    mots: [
      "accouchement",
      "administration hospitalière",
      "analyses biologiques",
      "assister",
      "audiologie",
      "audition",
      "autonomie motrice",
      "biochimie",
      "clinique",
      "contact patient",
      "dévouement",
      "encadrement soignant",
      "gestion des dossiers médicaux",
      "gestion hospitalière",
      "grossesse",
      "handicap",
      "hygiène hospitalière",
      "hématologie",
      "imagerie médicale",
      "kinésithérapie",
      "langage",
      "neuropsychologie",
      "orthopédie technique",
      "prothèses",
      "prothèses auditives",
      "prévention des risques",
      "périnatalité",
      "radiologie",
      "rééducation",
      "rééducation physique",
      "salubrité",
      "scanner / IRM",
      "soins de proximité",
      "soins infirmiers",
      "sport",
    ],
  },
  {
    domaine: "Tech / Ingénierie",
    label_ar: "التكنولوجيا والهندسة",
    mots: [
      "analyse de données",
      "applications",
      "architecture logicielle",
      "audit numérique",
      "automatisation d'usine",
      "business intelligence",
      "chimie industrielle",
      "circuits électroniques",
      "coder",
      "commande de systèmes",
      "conception industrielle",
      "connectivité",
      "cryptographie",
      "dessalement",
      "drones",
      "développement logiciel",
      "environnement",
      "gestion de projets numériques",
      "gouvernance des systèmes d'information",
      "grands systèmes",
      "génie civil maritime/aéroportuaire",
      "hacking éthique",
      "hyperfréquences",
      "industrie 4.0",
      "infrastructures de transport",
      "intelligence artificielle",
      "machine learning",
      "machines",
      "mécanique",
      "nanomatériaux",
      "navigation",
      "objets connectés (IoT)",
      "physique des matériaux",
      "production",
      "protéger les données",
      "prédictions",
      "puissance",
      "pétrochimie",
      "raffinage",
      "recherche fondamentale",
      "robotique",
      "robotique mobile",
      "réseaux 5G",
      "réseaux électriques",
      "résoudre des problèmes logiques",
      "spatial",
      "supervision industrielle",
      "systèmes cyber-physiques",
      "systèmes embarqués",
      "sécurité informatique",
      "traitement de l'eau",
      "télécoms",
      "écologie industrielle",
      "énergie électrique",
      "énergies renouvelables",
    ],
  },
  {
    domaine: "Sciences de la Nature",
    label_ar: "علوم الطبيعة",
    mots: [
      "agriculture",
      "agro-écologie",
      "biochimie fondamentale",
      "biologie moléculaire",
      "biotechnologie",
      "cultures",
      "foresterie",
      "gestion des terres",
      "génétique",
      "immunologie",
      "laboratoire",
      "microbiologie",
      "nature",
      "recherche biomédicale",
      "vivant",
      "écosystèmes",
    ],
  },
  {
    domaine: "Sciences exactes",
    label_ar: "العلوم الدقيقة",
    mots: [
      "actuariat",
      "algorithmes",
      "calcul",
      "data analytics",
      "logique abstraite",
      "modèles mathématiques",
      "optimisation",
      "recherche opérationnelle",
      "statistique",
    ],
  },
  {
    domaine: "Architecture",
    label_ar: "الهندسة المعمارية",
    mots: [
      "architecture",
      "bioclimatique",
      "bâtiment",
      "conception",
      "création visuelle",
      "design",
      "dessin",
      "patrimoine",
      "urbanisme",
    ],
  },
  {
    domaine: "Droit",
    label_ar: "الحقوق",
    mots: [
      "argumentation",
      "conseil juridique",
      "droit des affaires",
      "droit public",
      "droit pénal",
      "droits humains",
      "défense",
      "justice",
      "lois",
      "plaidoirie",
    ],
  },
  {
    domaine: "Sciences Humaines",
    label_ar: "العلوم الإنسانية",
    mots: [
      "FLE",
      "FOS",
      "anglais des affaires",
      "communication",
      "culture",
      "didactique",
      "enseignement",
      "enseignement des langues",
      "ingénierie pédagogique",
      "interprétation",
      "interprétation de conférence",
      "langues étrangères",
      "littérature",
      "littérature française",
      "multilinguisme",
      "traduction",
      "traduction technique",
      "voyages",
    ],
  },
  {
    domaine: "Business",
    label_ar: "الأعمال والتسيير",
    mots: [
      "audit bancaire",
      "audit financier",
      "commerce",
      "commerce extérieur",
      "comptabilité",
      "conduite du changement",
      "conjoncture",
      "contrôle de gestion",
      "coopération internationale",
      "data analytics économiques",
      "distribution",
      "douanes",
      "e-commerce",
      "entreprise",
      "finance",
      "finance bancaire",
      "gestion",
      "gestion des risques",
      "logistique internationale",
      "management",
      "management international",
      "marketing",
      "marketing digital",
      "mobilité académique",
      "recrutement",
      "relation client",
      "ressources humaines",
      "statistiques",
      "supply chain numérique",
      "vente",
      "économie",
      "économie monétaire",
      "économétrie",
      "économétrie appliquée",
    ],
  },
];
